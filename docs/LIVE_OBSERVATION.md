...
optional message-thread ID. The operator explicitly excluded Discord from
production and release gating on 2026-07-22; its unused compatibility transport
was removed from `dev` on 2026-08-08. Events are
...
redaction. Telegram is an external processor even when the gateway itself is
limited to authenticated tailnet and local-LAN ingress. Provider secrets use
Pydantic `SecretStr` and must arrive
...
The historical 2026-07-22 Discord/Telegram loopback evidence remains preserved
in `docs/VALIDATION.md`. Current Telegram checks cover message-thread targeting,
safe payload projection, HTTP 429, connection outage, and non-blocking failure
isolation. Separate checks pass allowlist denial, scoped nonce, expiration,
...
Discord is not a production release gate and has no current source/config path.
