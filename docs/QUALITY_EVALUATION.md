...
uses protocol `frontier-long-goal-v48` and contains one header, consecutive
...
`avatarforge-long-goal-v45` with consecutive checkpoints
...
invalid evidence. OpenCode is limited to 32 agent steps per checkpoint; reaching
that limit without a terminal clean checkpoint is a failure. The ten-hour limit
is one cumulative deadline for the complete
...
and compatible `patch` or `diff` aliases, but normalize all three to the raw
`input` expected by the Codex host. A correction retry is complete only when
the patch has begin/end markers and a concrete file operation. The local
Reviewer must scan all bounded evidence before deciding
...
material local finding returns the task to correction without spending a
