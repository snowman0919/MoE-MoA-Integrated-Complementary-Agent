...
## Corpus Check
- 250 files · ~346,383 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3293 nodes · 9448 edges · 178 communities (128 shown, 50 thin omitted)
- Extraction: 80% EXTRACTED · 20% INFERRED · 0% AMBIGUOUS · INFERRED: 1869 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `18a7ac6d`
...
- `test_loop_budget_overrides_merge_request_class_then_risk()` --uses--> `Settings`  [INFERRED]
  tests/test_config_auth.py → gateway/src/dgx_moa/config.py
- `test_weekly_jobs_use_requested_disabled_seoul_defaults()` --uses--> `Settings`  [INFERRED]
  tests/test_config_auth.py → gateway/src/dgx_moa/config.py
...
## Communities (178 total, 50 thin omitted)
...
Cohesion: 0.06
Nodes (62): model_validator, _bounded_ids(), _canonical(), _checkpoint_hash(), compact_session_active_state(), compile_execution_graph(), EdgeType, ExecutionGraph (+54 more)

### Community 2 - "Controller"
Cohesion: 0.07
Nodes (106): Controller, LoopAdmissionError, FrontierCollaborationResult, Phase, StrEnum, StateStore, asyncio, Exception (+98 more)
...
Cohesion: 0.08
Nodes (37): BaseModel, Connection, field_validator, model_validator, Path, Immutable, versioned Executor procedure; models may only recommend it., RuntimeSkill, SkillCandidateEvaluation (+29 more)

### Community 7 - "create_app"
Cohesion: 0.08
Nodes (69): create_app(), main(), get_settings(), Settings, continuation_correlation(), FakeLifecycleDriver, MockJudgeProvider, TestClient (+61 more)
...
Cohesion: 0.06
Nodes (50): command(), dashboard_telemetry(), event_count(), _gpu_values(), main(), memory_available(), _memory_values(), minimum_memory() (+42 more)

### Community 10 - "SessionState"
Cohesion: 0.07
Nodes (29): active_failures(), argument_paths(), compact_resolved_goal_history(), effective_objective(), has_mcp_server_failure(), pending_goal_prerequisites(), Any, BudgetName (+21 more)
...
### Community 18 - "specialists.py"
Cohesion: 0.10
Nodes (19): AcquireCallback, SpecialistRoutingConfig, _MockProvider, PlannerProvider, ABC, Any, AsyncBaseTransport, Exception (+11 more)
...
### Community 22 - "api.py"
Cohesion: 0.06
Nodes (33): ASGIApp, _chat_response_payload(), _coerce_responses_input_messages(), _coerce_responses_tools(), DrainMiddleware, DynamicRoleUnmanagedError, error_response(), has_matching_tool_result() (+25 more)
...
Cohesion: 0.20
Nodes (7): LifecycleLease, LifecycleRecord, LifecycleStore, Any, Connection, StaleTransitionError, LifecycleState
...
### Community 27 - "test_trace_v2.py"
Cohesion: 0.14
Nodes (32): validate_failure_record(), audit_traces(), export_trace(), final_status(), main(), Any, Path, Build bounded trajectory evidence; never source or hidden-reasoning archives. (+24 more)
...
Cohesion: 0.12
Nodes (20): DeclarativePolicyConfig, default_lifecycle_roles(), default_loop_budgets(), ExecutionGraphConfig, ExecutorSchedulingConfig, LifecycleRolePolicy, LiveObservationConfig, LoopEngineeringPolicy (+12 more)
...
Cohesion: 0.09
Nodes (36): clean_tool_output(), embedded_tool_exit_code(), FrontierRequiredUnavailable, JudgeCorrectionRequired, JudgeRequired, normalize_tool_result(), RuntimeError, Keep tool evidence structured; tolerate OpenCode-compatible string payloads. (+28 more)
...
### Community 45 - "build_runtime_evidence_snapshot"
Cohesion: 0.19
Nodes (25): build_runtime_evidence_snapshot(), canonical_request_input(), project_role_context(), ProjectionRole, ProjectionStage, Versioned source of truth from which every role context is independently…, runtime_evidence_item(), RuntimeEvidenceSnapshot (+17 more)
...
### Community 47 - "managed_http_client"
Cohesion: 0.12
Nodes (21): make_http_client(), managed_http_client(), AsyncBaseTransport, AsyncClient, Shared HTTPX client helpers used across gateway modules., Create a single AsyncClient with optional timeout/transport overrides., Create one request-scoped AsyncClient and guarantee closure., OpenCodeGoExecutorProvider (+13 more)
...
### Community 55 - "test_goal_tooling.py"
Cohesion: 0.13
Nodes (24): evaluate(), main(), Any, Path, register(), compare(), cooldown_active(), main() (+16 more)
...
Cohesion: 0.13
Nodes (14): Path, settings(), stub_provider(), Any, MonkeyPatch, Path, StubFlashExecutor, test_admin_dashboard_runs_bounded_custom_provider_codex() (+6 more)

### Community 62 - "providers.py"
Cohesion: 0.14
Nodes (13): mistral_messages(), mistral_tool_call_id(), OwnedByteStream, parse_json_content(), Any, AsyncClient, Response, Fit local specialist output to the context actually served by vLLM. (+5 more)
...
### Community 66 - "redact"
Cohesion: 0.21
Nodes (16): compress_messages(), compress_text(), message_fingerprint(), Any, redact(), test_compression_keeps_assistant_preceding_retained_tool_results(), test_default_tool_output_budget_preserves_small_source_files(), test_redaction_and_compression() (+8 more)

### Community 67 - "PolicyEngine"
Cohesion: 0.26
Nodes (12): PolicyBlocked, PolicyEngine, PolicySet, policy_set(), test_controller_applies_policy_roles_limits_and_trace(), test_controller_blocks_missing_policy_approval_and_persists_reason(), test_controller_enforces_tool_deny_and_evidence_field_redaction(), test_policy_engine_traces_versioned_aggregated_decision() (+4 more)

### Community 68 - "LiveDashboardHub"
Cohesion: 0.18
Nodes (9): LiveDashboardHub, Any, Bounded API-key-scoped projection of durable runtime events., _Subscriber, Queue, graph_request(), asyncio, test_live_dashboard_isolates_keys_and_redacts_operator_stream() (+1 more)
...
Cohesion: 0.25
Nodes (5): LifecycleAutomationStatus, Path, Row, read_automation_status(), _sanitize_failure_class()
...
### Community 135 - "._observe"
Cohesion: 0.22
Nodes (7): classify_failure(), DuplicateFailedCall, failure_family(), fingerprint(), test_duplicate_failed_call_ignores_call_id(), test_failure_classification(), test_parallel_tool_results_match_their_calls()
...
- **Why does `create_app()` connect `create_app` to `frontier.py`, `ValueError`, `Controller`, `StubProvider`, `test_streaming.py`, `SkillRegistry`, `._observe`, `ApiKeyStore`, `UsageStore`, `SessionState`, `remote_judge.py`, `observation.py`, `ModelProvider`, `replay.py`, `KnowledgeRegistry`, `asyncio`, `specialists.py`, `ProfileManager`, `schemas.py`, `api.py`, `LifecycleStore`, `test_trace_v2.py`, `test_state_routing.py`, `weekly.py`, `test_training.py`, `TrainingStore`, `ExecutorScheduler`, `controller.py`, `LifecycleCoordinator`, `datetime`, `managed_http_client`, `training.py`, `SystemdLifecycleDriver`, `security.py`, `ModelConfig`, `conftest.py`, `providers.py`, `redact`, `PolicyEngine`, `LiveDashboardHub`, `RuntimeMetrics`, `ArchiveRegistry`, `LifecycleDriver`, `validate-live-client-matrix.py`, `WeeklyScheduler`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Why does `Controller` connect `Controller` to `frontier.py`, `ValueError`, `context_projection.py`, `SkillRegistry`, `create_app`, `._observe`, `UsageStore`, `SessionState`, `remote_judge.py`, `ModelProvider`, `replay.py`, `KnowledgeRegistry`, `specialists.py`, `schemas.py`, `api.py`, `main`, `test_policy_redacts_specialist_state_event_and_evaluation_boundaries`, `controller.py`, `build_runtime_evidence_snapshot`, `managed_http_client`, `ModelConfig`, `PolicyEngine`, `media_assets`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `UsageStore` connect `UsageStore` to `Controller`, `controller.py`, `create_app`, `LifecycleCoordinator`, `lifecycle.py`, `test_lifecycle.py`, `api.py`, `security.py`?**
