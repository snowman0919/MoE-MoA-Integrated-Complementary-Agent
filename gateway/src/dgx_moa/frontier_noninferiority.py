...
def analyze(
    ...
    *,
    ...
    rows = load_rows(path, repeats, tasks)
    repeat_ids = sorted({key[0] for key in rows})
    result: dict[str, Any] = {
    ...
    for variant in VARIANTS[1:]:
        ...
        if quality_pairs == expected_pairs:
            quality_lower, quality_upper = stratified_bootstrap(
                quality,
                samples=bootstrap_samples,
                seed=seed,
                tasks=tasks,
            )
            quality_bounds = (quality_lower, quality_upper)
            quality_result = verdict_quality(quality_lower, quality_upper)
        else:
            quality_bounds = (None, None)
