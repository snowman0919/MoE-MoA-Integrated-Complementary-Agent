...
set -Eeuo pipefail
cd "$(dirname "$0")/.."
systemctl --user stop dgx-moa-resident.target dgx-moa-judge.target \
...
systemctl --user disable dgx-moa-gateway.service dgx-moa-loopback.socket \
  dgx-moa-resident.target dgx-moa-judge.target 2>/dev/null || true
