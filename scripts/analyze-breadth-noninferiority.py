...
BASE = runpy.run_path(str(Path(__file__).with_name("analyze-frontier-noninferiority.py")))
CATEGORIES = {
...
def analyze(path: Path, *, repeats: int = 10, bootstrap_samples: int = 10_000):
    lines = [json.loads(line) for line in path.read_text().splitlines() if line.strip()]
    categories = {}
    base_globals = BASE["analyze"].__globals__
    original_tasks, original_seed = base_globals["TASKS"], base_globals["SEED"]
    try:
        for category, tasks in CATEGORIES.items():
            with tempfile.NamedTemporaryFile("w", suffix=".jsonl") as handle:
                ...
                handle.flush()
                base_globals["TASKS"] = tasks
                base_globals["SEED"] = SEED
        ...
        base_globals["TASKS"], base_globals["SEED"] = original_tasks, original_seed
    return {
