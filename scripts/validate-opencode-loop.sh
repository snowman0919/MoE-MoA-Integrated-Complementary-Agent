...
{"model":"dgx-moa","messages":[{"role":"user","content":"Call read_file once for /tmp/dgx-moa-validation.txt."}],"tools":[{"type":"function","function":{"name":"read_file","description":"Read a file","parameters":{"type":"object","properties":{"path":{"type":"string"}},"required":["path"],"additionalProperties":false}}}],"tool_choice":"required"}
JSON
...
curl -fsS -N "${auth[@]}" "${identity[@]}" -H "X-Session-ID: $stream_session" \
  ...
  -d '{"model":"dgx-moa","stream":true,"messages":[{"role":"user","content":"Reply READY."}]}' \
