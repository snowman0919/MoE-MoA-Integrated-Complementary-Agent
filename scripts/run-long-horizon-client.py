...
QUALITY = runpy.run_path(str(PROJECT / "scripts/run-client-quality-matrix.py"))
RUNTIME = runpy.run_path(str(PROJECT / "scripts/validate-isolated-sglang-runtime.py"))
            ...
            if row.get("type") == "tool_use":
                ...
                if part.get("tool") == "read":
                    ...
                    read_fingerprints.append(
                        sha256_text(json.dumps(state.get("input") or {}, sort_keys=True))
                    )
...
def progress_state(state: Path, index: int) -> dict[str, Any]:
    ...
    if value.get("premature_completion") is not False:
        raise RuntimeError("premature_completion")
    return value
...
def run_checkpoint(
    ...
    provider = provider_metrics(args.state_db, started, completed)
    ...
    checkpoint = {
        ...
        "unjustified_repeated_reads": max(
            ...
            int(progress.get("unjustified_repeated_reads", 0)),
        ),
