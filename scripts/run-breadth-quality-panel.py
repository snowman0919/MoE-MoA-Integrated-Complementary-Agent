#!/usr/bin/env python3
"""Run the frozen Scientific Reasoning and General repository-agent panel."""
...
BASE = runpy.run_path(str(Path(__file__).with_name("run-client-quality-matrix.py")))
Task = BASE["Task"]
block = BASE["block"]
...
BASE["TASKS"] = TASKS
BASE["TASK_BY_SLUG"] = {task.slug: task for task in TASKS}
BASE["HIDDEN_CHECKS"] = HIDDEN_CHECKS
BASE["main"].__globals__.update(
    TASKS=TASKS,
    TASK_BY_SLUG=BASE["TASK_BY_SLUG"],
